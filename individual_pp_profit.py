...
df_installed = pd.read_excel("output\installed_capacity_6.1_90000.xlsx",header=0) 
...
revenue_yr_series = pd.DataFrame(index=[pp.plant_type for pp in pp_list])
...
for year in range(n):
    ...
    for q0 in range(len(q0t)): ##length is 4, loop through.
        ...
        for solar_index in range(len(availability_levels_solar)):##4 levels of solar availability.
            ...
            for wind_index in range(len(availability_levels_wind)):
                ...
                df_pp['revenue'] = df_pp.dispatched_KWHs * eq_price
                df_pp['revenue_yr'] += df_pp.revenue
    ...
    revenue_yr_series[str(year)] = pd.concat([yr_revenue], axis=1, sort=False)
    
    continue
#dispatched_yr_series.to_excel("output\yr_dispatched_series_20190401.xlsx")   
revenue_yr_series.to_excel("output/revenue_yr_series.xlsx")     
print(dispatched_yr_series)
#fun_plot(revenue_yr_series,ylabel='(KWh)',title='electricity dispached (KWh)',yr=year) ##plot dispatch for whole 70yr.
# multiple line plot
plt.plot(revenue_yr_series.loc['coal'], color='saddlebrown', linewidth=2, label= 'coal')
plt.plot(revenue_yr_series.loc['gas'], color='skyblue', linewidth=2, label= 'gas')
plt.plot(revenue_yr_series.loc['solar'], color='gold', linewidth=2, label= 'solar')
plt.plot(revenue_yr_series.loc['wind'], color='forestgreen', linewidth=2, label= 'wind')
plt.plot(revenue_yr_series.loc['CN_baseload'], color='silver', linewidth=2, label= 'CN_baseload')
plt.plot(revenue_yr_series.loc['biomass'], color='coral', linewidth=2, label= 'biomass')

plt.legend()
plt.title("Yearly revenue", fontsize=16)
plt.xlabel("years")
plt.ylabel("euro cent")
